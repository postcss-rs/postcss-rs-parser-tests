...
console.log(generate(root, 0, file ));
...
function generateRule(node, level, source) {
  ...
  let string = "";
  string += `${" ".repeat(level * 2)}Rule@${start.offset}..${end.offset}\n`;
...
function generateAtRule(node, level, source) {
  ...
  let string = "";
  string += `${" ".repeat(level * 2)}AtRule@${start.offset}..${end.offset}\n`;
...
function generateDeclaration(node, level, source) {
  ...
  let string = "";
  string += `${" ".repeat(level * 2)}Declaration@${start.offset}..${end.offset}\n`;
